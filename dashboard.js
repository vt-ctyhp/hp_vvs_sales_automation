...
const KPI_CARDS_ONCAL = [
  ...
  // Consults: Scheduled + Attended
  { key:'consultationsScheduled',   label:'Consultations (Scheduled)',   fmt:'0', upGood:true,  drill:'cal_consults' },
  { key:'consultationsAttended',    label:'Consultations (Attended)',    fmt:'0', upGood:true,  drill:'cal_consults_attended' },

  // DVs: Scheduled + Attended
  { key:'dvsScheduled',             label:'Diam View (Scheduled)',       fmt:'0', upGood:true,  drill:'cal_dvs' },
  { key:'dvsAttended',              label:'Diam View (Attended)',        fmt:'0', upGood:true,  drill:'cal_dvs_attended' }
];
...
function buildMetricsView_() {
  ...
  const iCOS    = findCol_(MH, ['Custom Order Status','COS'], false);               
  const iIPS    = findCol_(MH, ['In Production Status','IPS'], false);             
  ...
  const headers = [
    ...
    'Custom Order Status','In Production Status'  
  ];
...
function writeDashboard_() {
  ...
  if (typeof KPI_CARDS_PRODUCTION !== 'undefined' && KPI_CARDS_PRODUCTION.length) {
    dash.getRange(cardRow, 1)
        .setValue('Production Stage Breakdown')
        .setFontWeight('bold').setFontSize(12);
    renderScorecardsCompact_({
      sheet: dash, startRow: cardRow + 1, startCol: 1,
      cardsPerRow: 10, cardWidth: 2, cardHeight: 4, hGap: 1, vGap: 1,
      spec: KPI_CARDS_PRODUCTION, cur, prv, addSparkline: false, anchors
    });
    cardRow += adv(KPI_CARDS_PRODUCTION.length);
  }
...
function computeKpis_(winStart, winEnd, brandFilter, repFilter, master, masterHeader, metrics, metricsHeader) {
  ...
  const noTouch48 = metrics.filter(r => {
    if (!(isBrand(r) && isRep(r))) return false;
    const stageRaw = r[xi['Sales Stage']];
    if (isWonOrLostStage_(stageRaw)) return false;
    // FIX: chỉ count leads active trong 90 ngày qua
    if (!isActiveAsOf_(r, xi, asOf)) return false;
    // FIX: bỏ qua records không có Last Touch date
    const last = r[xi['Last Touch (Root Index)']];
    if (!(last instanceof Date)) return false;
    return lastTouchHoursAgo_(r, xi, asOf) > 48;
  }).length;
  ...
  const overduePr = metrics.filter(r => {
    if (!(isBrand(r) && isRep(r) && isActiveAsOf_(r, xi, asOf))) return false;
    // FIX: exclude orders đã completed
    const stage = String(r[xi['Sales Stage']]||'').trim().toUpperCase();
    if (stage.includes('ORDER COMPLETED') || stage.includes('WON')) return false;
    const due = r[xi['Prod Deadline']];
    return (due instanceof Date) && startOfDay_(due) < startOfDay_(asOf);
  }).length;
  ...
  return {
    ...
    ...(typeof P15_computeProductionBreakdown === 'function'
    ? P15_computeProductionBreakdown(metrics, xi, brandFilter, repFilter)
    : {})
  };
...
function computeScheduleKpis_(winStart, winEnd, brandFilter, repFilter, master, mH) {
  ...
  // Accept Scheduled OR Completed for the "On Calendar" KPIs only
  const isSchedOrCompleted = (r) => {
    if (iStatus < 0) return true;
    const s = String(r[iStatus] || '').trim().toLowerCase();
    return s === 'scheduled' || s === 'completed';
  };
  // Accept Scheduled / Completed / Cancel(l)ed / No-Show for On-Calendar KPIs
  const isSchedCompCancelNoShow = (r) => {
    if (iStatus < 0) return true;
    const s = String(r[iStatus] || '').trim().toLowerCase();
    return ['scheduled','completed','cancelled','canceled','no-show','no show'].includes(s);
  };
  // Attended = exclude Canceled / Cancelled / No-Show
  const isNotCancelNoShow = (r) => {
    if (iStatus < 0) return true;
    const s = String(r[iStatus] || '').trim().toLowerCase();
    return !(/cancelled|canceled/.test(s) || /no[-\s]?show/.test(s));
  };
  ...
  const rowsSchedInWin = master.filter(r =>
    matchesBR(r) &&
    inWinDate(r[iVDate]) &&
    isSchedCompCancelNoShow(r) &&
    isApptType(r)
  );
  // "Attended": same window/brand/rep/type, but exclude cancels & no-shows
  // const rowsAttendInWin = master.filter(r =>
  //   matchesBR(r) &&
  //   inWinDate(r[iVDate]) &&
  //   isApptType(r) &&
  //   isNotCancelNoShow(r)
  ...
  const consultationsAttended = rowsAttendInWin.filter(isConsult).length;
  const dvsAttended           = rowsAttendInWin.filter(isDV).length;
  ...
  return {
    ...
    // On Calendar (Scheduled)
    consultationsScheduled, dvsScheduled,

    // On Calendar (Attended) — NEW
    consultationsAttended, dvsAttended,
...
{
  const schedRows = master.filter(r =>
    matchBR(r) &&
    inWin(r[iVDate]) &&
    (iStatus < 0 || ['scheduled','completed','cancelled','canceled','no-show','no show']
      .includes(String(r[iStatus] || '').trim().toLowerCase())) &&
  ...
  // Diamond Viewings (Scheduled)
  {
    const rows = schedRows
      .filter(r => String(r[iVType]||'').trim().toLowerCase()==='diamond viewing')
      .map(r => [r[iRoot], (iCust>=0?r[iCust]:''), r[iVDate], r[iVType], r[iRep], r[iBrand]]);
    addSection('On Calendar — Diam View (Scheduled)', 'cal_dvs',
              rows, ['RootApptID','Customer','Visit Date','Visit Type','Rep','Brand']);
  }

  // Build "Attended" rows: exclude cancels & no-shows
  const attendRows = master.filter(r =>
    matchBR(r) &&
    inWin(r[iVDate]) &&
    (String(r[iVType] || '').trim() !== '') &&
    (iStatus < 0 || !(/cancelled|canceled/.test(String(r[iStatus]||'').toLowerCase()) ||
                      /no[-\s]?show/.test(String(r[iStatus]||'').toLowerCase())))
  );
  ...
  // Consultations (Attended)
  {
    const rows = attendRows
      .filter(r => String(r[iVType]||'').trim().toLowerCase()==='appointment')
      .map(r => [r[iRoot], (iCust>=0?r[iCust]:''), r[iVDate], r[iVType], r[iRep], r[iBrand]]);
    addSection('On Calendar — Consultations (Attended)', 'cal_consults_attended',
              rows, ['RootApptID','Customer','Visit Date','Visit Type','Rep','Brand']);
  }
  ...
  // Diam View (Attended)
  {
    const rows = attendRows
      .filter(r => String(r[iVType]||'').trim().toLowerCase()==='diamond viewing')
      .map(r => [r[iRoot], (iCust>=0?r[iCust]:''), r[iVDate], r[iVType], r[iRep], r[iBrand]]);
    addSection('On Calendar — Diam View (Attended)', 'cal_dvs_attended',
              rows, ['RootApptID','Customer','Visit Date','Visit Type','Rep','Brand']);
  }
  ...
  {
    const uniqRoots = new Set(master.filter(r =>
      matchBR(r) &&
      inWin(r[iVDate]) &&
      (iStatus < 0 || ['scheduled','completed','cancelled','canceled','no-show','no show']
        .includes(String(r[iStatus] || '').trim().toLowerCase())) &&
  ...
  const rowsNoTouch48 = metrics.filter(r => {
    ...
    const stageRaw = r[xi['Sales Stage']];
    if (isWonOrLostStage_(stageRaw)) return false;
    if (!isActive(r)) return false; // FIX: 90-day filter
    const last = r[xi['Last Touch (Root Index)']];
    if (!(last instanceof Date)) return false; // FIX: bỏ qua NULL
    return lastTouchHoursAgo_(r, xi, asOf) > 48;
  }).map(r => [r[xi['RootApptID']], r[xi['Customer Name']], r[xi['Last Touch (Root Index)']], r[xi['Assigned Rep']]]);
  ...
  if (typeof P15_STAGES !== 'undefined' && xi['Custom Order Status'] != null) {
    P15_STAGES.forEach(stage => {
      const stageKey  = 'prod_stage_' + stage.toLowerCase().replace(/\s+/g, '_');
      const stageRows = (typeof P15_buildProductionDrillRows === 'function')
        ? P15_buildProductionDrillRows(metrics, xi, stage, brand, rep)
        : [];
      addSection(
        'Production Stage — ' + stage + ' (as-of window end)',
        stageKey,
        stageRows,
        ['RootApptID','Customer','Rep','Stage','Stage Updated At (from log)',
         'Days in Stage','Order Total','Prod Deadline']
      );
    });
  }
...
function backupBeforeCleanup() {
  ...
  // Tạo copy vào cùng folder với file gốc
  const originalFile = DriveApp.getFileById(ss.getId());
  const parents = originalFile.getParents();
  const folder = parents.hasNext() ? parents.next() : DriveApp.getRootFolder();
  
  const copy = originalFile.makeCopy(backupName, folder);
  
  SpreadsheetApp.getUi().alert(
    `✅ Backup tạo thành công!\n\nTên: ${backupName}\nFolder: ${folder.getName()}\nID: ${copy.getId()}\n\nBây giờ có thể chạy audit an toàn.`
  );
  
  console.log(`Backup ID: ${copy.getId()}`);
}

function auditOrphanData() {
  const ss = SpreadsheetApp.getActive();
  const log = [];

  // 1) Master — thu thập valid IDs
  const master = ss.getSheetByName('00_Master Appointments').getDataRange().getValues();
  const mH = master.shift().map(h => String(h||'').trim());
  const iRoot = mH.findIndex(h => /root.*id|rootappt/i.test(h));
  const iAppt = mH.findIndex(h => /^appt_id$|^apptid$|^appointment.?id$/i.test(h));

  const validRoots = new Set(master.map(r => String(r[iRoot]||'')).filter(Boolean));
  const validAppts = new Set(master.map(r => String(r[iAppt]||'')).filter(Boolean));
  log.push(`✅ Master: ${master.length} rows | ${validRoots.size} unique RootApptIDs | ${validAppts.size} unique ApptIDs`);

  // 2) Root Index
  const rootIdx = ss.getSheetByName('07_Root_Index');
  if (rootIdx) {
    const ri = rootIdx.getDataRange().getValues();
    const rih = ri.shift().map(h => String(h||'').trim());
    const col = rih.findIndex(h => /root.*id|rootappt/i.test(h));
    const orphans = ri.filter(r => String(r[col]||'').trim() && !validRoots.has(String(r[col]).trim()));
    log.push(`${orphans.length > 0 ? '🔴' : '✅'} Root Index: ${ri.length} rows | ${orphans.length} ORPHANED (không có trong Master)`);
  ...
  // 4) Status Log
  const statusLog = ss.getSheetByName('03_Client_Status_Log');
  if (statusLog) {
    const sl = statusLog.getDataRange().getValues();
    const slh = sl.shift().map(h => String(h||'').trim());
    const col = slh.findIndex(h => /^appt_id$|^apptid$|^appointment.?id$/i.test(h));
    if (col >= 0) {
      const orphans = sl.filter(r => String(r[col]||'').trim() && !validAppts.has(String(r[col]).trim()));
      log.push(`${orphans.length > 0 ? '🔴' : '✅'} Status Log: ${sl.length} rows | ${orphans.length} ORPHANED appt IDs`);
    } else {
      log.push(`⚠️  Status Log: không tìm thấy cột APPT_ID — kiểm tra tên cột`);
    }
  } else {
    log.push(`⚠️  Sheet 03_Client_Status_Log KHÔNG TÌM THẤY`);
  }

  // 5) Metrics View — stale cache
  const metrics = ss.getSheetByName('100_Metrics_View');
  if (metrics) {
    const mv = metrics.getDataRange().getValues();
    const mvh = mv.shift().map(h => String(h||'').trim());
    const col = mvh.findIndex(h => /^rootapptid$/i.test(h));
    const stale = mv.filter(r => String(r[col]||'').trim() && !validRoots.has(String(r[col]).trim()));
    log.push(`${stale.length > 0 ? '🔴' : '✅'} Metrics View: ${mv.length} rows | ${stale.length} STALE rows (cache cũ chưa rebuild)`);

    // 6) No-Touch breakdown
    const xi = {};
    mvh.forEach((h,i) => xi[h] = i);
    const nullTouch = mv.filter(r => !(r[xi['Last Touch (Root Index)']] instanceof Date)).length;
    const real48h   = mv.filter(r => {
      const last = r[xi['Last Touch (Root Index)']];
      return (last instanceof Date) && ((new Date() - last) / 3600000 > 48);
    }).length;
    log.push(`\n📊 No-Touch breakdown (No-touch >48h = ${nullTouch + real48h}):`);
    log.push(`   NULL Last Touch (không có record → tự động tính là no-touch): ${nullTouch}`);
    log.push(`   Có Last Touch nhưng thực sự > 48h:                            ${real48h}`);

    // 7) Production Overdue breakdown
    const stageCol = xi['Sales Stage'] ?? -1;
    const prodDueCol = xi['Prod Deadline'] ?? -1;
    if (prodDueCol >= 0) {
      const today = new Date();
      const allOverdue = mv.filter(r => {
        const due = r[prodDueCol];
        return (due instanceof Date) && due < today;
      });
      const alreadyDone = allOverdue.filter(r => {
        const stage = String(r[stageCol]||'').toUpperCase();
        return stage.includes('ORDER COMPLETED') || stage.includes('WON');
      });
      log.push(`\n📊 Production Overdue breakdown (hiện tại dashboard = ${allOverdue.length}):`);
      log.push(`   Thực sự overdue (chưa completed):  ${allOverdue.length - alreadyDone.length}`);
      log.push(`   Đã completed nhưng vẫn bị đếm:     ${alreadyDone.length}`);
    }
  } else {
    log.push(`⚠️  Sheet 100_Metrics_View KHÔNG TÌM THẤY — cần chạy buildMetricsView_ trước`);
  }

  // Ghi ra sheet Audit_Orphans
  const out = ss.getSheetByName('Audit_Orphans') || ss.insertSheet('Audit_Orphans');
  out.clear();
  out.getRange(1,1).setValue('🔍 Orphan Audit — ' + new Date().toLocaleString()).setFontWeight('bold');
  out.getRange(2,1,log.length,1).setValues(log.map(l=>[l]));
  out.setColumnWidth(1, 700);
  out.autoResizeRows(1, log.length + 1);

  console.log('✅ Audit xong! Xem sheet Audit_Orphans');
}

function findSource349B() {
  const ss = SpreadsheetApp.getActive();
  const target = 349999999999;
  const results = [];
  ...
  ss.getSheets().forEach(sh => {
    ...
    data.forEach((row, ri) => {
      row.forEach((cell, ci) => {
        const n = Number(cell);
        // Tìm giá trị lớn hơn 100 triệu trong bất kỳ cell nào
        if (isFinite(n) && n > 100000000) {
          results.push({
            sheet: sh.getName(),
            cell: `Row ${ri+1} Col ${ci+1}`,
            value: n,
            raw: String(cell).slice(0, 50)
          });
        }
      });
    });
  ...
  if (results.length === 0) {
    console.log('✅ Không tìm thấy giá trị nào > 100 triệu trong file này');
    console.log('→ Giá trị $349B có thể đến từ file Payments ledger bên ngoài');
  } else {
    console.log(`🔴 Tìm thấy ${results.length} cells bất thường:`);
    results.forEach(r => console.log(JSON.stringify(r)));
  }
}

function debugLive349B() {
  const ss = SpreadsheetApp.getActive();
  const dash = ss.getSheetByName('00_Dashboard');

  const start = new Date(dash.getRange('B2').getValue());
  const end   = new Date(dash.getRange('D2').getValue());
  ...
  const master  = ss.getSheetByName('00_Master Appointments').getDataRange().getValues();
  const mH      = master.shift().map(x => String(x||'').trim());
  const metrics = ss.getSheetByName('100_Metrics_View').getDataRange().getValues();
  const xH      = metrics.shift().map(x => String(x||'').trim());

  // Build allowedRootsSet cho previous window (giống hệt computeKpis_)
  const iBrand = mH.findIndex(h => /^brand$/i.test(h));
  const iRep   = mH.findIndex(h => /assigned rep|sales rep/i.test(h));
  const iRoot  = mH.findIndex(h => /rootappt/i.test(h));

  const brandFilter = String(dash.getRange('H1').getValue()||'').trim();
  const repFilter   = String(dash.getRange('H2').getValue()||'').trim();

  const allowedRootsSet = new Set(
    master
      .filter(r =>
        (!brandFilter || String(r[iBrand]||'').trim() === brandFilter) &&
        (!repFilter   || String(r[iRep]  ||'').trim() === repFilter))
  ...
  console.log(`Brand filter: "${brandFilter}" | Rep filter: "${repFilter}"`);
  console.log(`allowedRootsSet size: ${allowedRootsSet.size}`);
  console.log(`Sample roots: ${[...allowedRootsSet].slice(0,5).join(', ')}`);

  // Gọi trực tiếp hàm tính All Payments
  const result = sumAllDepositsAmountInWindow_(prevStart, prevEnd, allowedRootsSet);
  console.log(`\nsumAllDepositsAmountInWindow_ trả về: $${result.toLocaleString()}`);

  // Kiểm tra nếu allowedRootsSet có chứa fake IDs không
  const fakeIds = ['412131321313','4121313123123','44343253252','141242141','14131212412'];
  fakeIds.forEach(id => {
    if (allowedRootsSet.has(id)) console.log(`🚨 FAKE ID trong allowedRootsSet: ${id}`);
  });

  // Check _IntakeQueue có ảnh hưởng không
  const iq = ss.getSheetByName('_IntakeQueue');
  if (iq) {
    console.log(`\n_IntakeQueue: ${iq.getLastRow()} rows × ${iq.getLastColumn()} cols`);
    const iqH = iq.getRange(1,1,1,iq.getLastColumn()).getValues()[0];
    console.log(`Headers: ${iqH.map((h,i)=>`[${i}]${h}`).join(' | ')}`);
  }
}

function removeFakeIdsFromMaster() {
  const ss = SpreadsheetApp.getActive();
  const sh = ss.getSheetByName('00_Master Appointments');
  const data = sh.getDataRange().getValues();
  const headers = data[0].map(h => String(h||'').trim());
  const iRoot = headers.findIndex(h => /rootappt/i.test(h));

  const fakeIds = new Set([
    '412131321313', '4121313123123', '44343253252', 
    '141242141', '14131212412', ' ' // blank/space cũng là fake
  ]);

  // Tìm các dòng cần xóa (duyệt từ dưới lên)
  const toDelete = [];
  for (let i = data.length - 1; i >= 1; i--) {
    const rootVal = String(data[i][iRoot]||'').trim();
    if (fakeIds.has(rootVal) || rootVal === '') {
      toDelete.push(i + 1); // convert to sheet row (1-indexed)
    }
  }

  if (toDelete.length === 0) {
    console.log('✅ Không tìm thấy fake ID nào trong Master');
    return;
  }

  console.log(`🔴 Tìm thấy ${toDelete.length} dòng cần xóa: rows ${toDelete.join(', ')}`);
  
  // Xóa từng dòng (đã sort từ dưới lên nên index không bị lệch)
  toDelete.forEach(rowNum => {
    console.log(`Xóa row ${rowNum}: RootApptID = ${sh.getRange(rowNum, iRoot+1).getValue()}`);
    sh.deleteRow(rowNum);
  });

  console.log(`✅ Đã xóa ${toDelete.length} fake rows khỏi Master`);
}

function clearBadHistoryAndRebuild() {
  const ss = SpreadsheetApp.getActive();
  const dash = ss.getSheetByName('00_Dashboard');

  // Xóa toàn bộ KPI history (cột AA = 27 trở đi, từ row 6 xuống)
  const lastRow = dash.getLastRow();
  if (lastRow >= 6) {
    dash.getRange(6, 27, lastRow - 5, 20).clearContent();
    console.log(`✅ Đã xóa KPI history từ row 6 đến ${lastRow}`);
  }

  console.log('Chạy rebuild...');
  runOnceToBuildAll();
  console.log('✅ Rebuild xong — kiểm tra dashboard');
}

function checkStatusValues() {
  const ss = SpreadsheetApp.getActive();
  const master = ss.getSheetByName('00_Master Appointments').getDataRange().getValues();
  const headers = master.shift().map(h => String(h||'').trim());
  const iStatus = headers.findIndex(h => /^status$|appt status/i.test(h));
  const iVType  = headers.findIndex(h => /visit type/i.test(h));
  
  // Đếm tất cả status values cho Diamond Viewing và Appointment
  const counts = {};
  master.forEach(r => {
    const type = String(r[iVType]||'').trim().toLowerCase();
    const stat = String(r[iStatus]||'').trim();
    if (!['appointment','diamond viewing'].includes(type)) return;
    const key = `[${type}] "${stat}"`;
...
function clearBadHistory() {
  const dash = SpreadsheetApp.getActive().getSheetByName('00_Dashboard');
  const lastRow = dash.getLastRow();
  if (lastRow >= 6) {
    dash.getRange(6, 27, lastRow - 5, 20).clearContent();
    console.log('✅ Đã xóa KPI history');
  }
}