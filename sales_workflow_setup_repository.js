...
function swDefaultTemplates_() {
  return [
    ...
    [SW_TASKS.DATA_CLEANUP_REVIEW, 'Review Stale Customer Data', 'Review this stale customer profile, verify current ownership and operational status, then submit the proposed cleanup update. The record is not changed until the paired Client Advisor/JOC confirmation is complete.', 'Customer: {{customerName}}\nClient Advisor: {{assignedRep}}\nJOC: {{assistedRep}}\nCurrent stage: {{salesStage}}\nConversion: {{conversionStatus}}\nNext steps: {{nextSteps}}', 'Client Status Report', '{{reportUrl}}', '[{"id":"owners_verified","label":"Client Advisor and JOC ownership are correct or noted","required":true},{"id":"contact_verified","label":"Customer contact information was reviewed","required":true},{"id":"ops_status_reviewed","label":"SO, payment, 3D, wax, and diamond status were reviewed","required":true},{"id":"next_steps_current","label":"Next steps/status notes are current","required":true}]', 'Submit Cleanup Proposal'],
    ...
    [SW_TASKS.DATA_CLEANUP_REVISE, 'Revise Customer Data Cleanup', 'The paired reviewer returned this cleanup update. Review their reason, revise the proposed customer status/data, and resubmit for confirmation.', 'Customer: {{customerName}}\nReturned reason: {{cleanupReturnReason}}\nPrevious proposal: {{cleanupProposalSummary}}', 'Client Status Report', '{{reportUrl}}', '[{"id":"return_reason_reviewed","label":"Reviewed the return reason from the other owner","required":true},{"id":"proposal_revised","label":"Revised or reconfirmed the cleanup proposal","required":true}]', 'Resubmit Cleanup']
  ];
}
