...
function getActiveMasterPreview(){
  ...
  let rowRich = [];
  try {
    rowRich = rowRange.getRichTextValues()[0] || [];
  } catch (_) {
    rowRich = [];
  }

  const getValue = (name) => (H[name] != null ? rowValues[H[name]] : '');
  const getDisplay = (name) => (H[name] != null ? rowDisplay[H[name]] : '');
  const getLinkOrDisplay = (name) => {
    const idx = H[name];
    if (idx == null) return '';
    ...
    const display = rowDisplay[idx];
    if (display != null && display !== '') return display;
    const raw = rowValues[idx];
    return raw != null ? String(raw) : '';
  };

  const brandRaw = getValue('Brand');
  const brand = String(brandRaw || '').toUpperCase();
  const soRaw = String(getDisplay('SO#') || getValue('SO#') || '').trim();
  const so = soRaw.replace(/^'/, '');
  const odooUrl = getLinkOrDisplay('Odoo SO URL') || getLinkOrDisplay('SO URL') || getLinkOrDisplay('Odoo Link');

  const customer = getValue('Customer Name') || getValue('Customer') || getValue('Name');
  const email = getValue('EmailLower') || getValue('Email');
  const phone = getValue('PhoneNorm') || getValue('Phone');
  const linkedAtRaw = H['SO Linked At'] != null ? getValue('SO Linked At') : '';

  const hasLinkedSO = !!(so || linkedAtRaw);
  const tz = Session.getScriptTimeZone?.() || 'America/Los_Angeles';
  const linkedAtDisplay = linkedAtRaw instanceof Date && !isNaN(linkedAtRaw)
    ? Utilities.formatDate(new Date(linkedAtRaw), tz, 'MMM d, yyyy h:mm a z')
    : (linkedAtRaw ? String(linkedAtRaw) : '');
  ...
  return {
    ...
    existingBrand: brand,
    existingSo: so,
    existingLinkedAtDisplay: linkedAtDisplay
  };
}
...
function checkSOConflicts(payload){
  ...
  if (!/^\d{2}\.\d{4,5}$/.test(so))   return { existsInMaster:false, existsInOrders:false };
...
function saveAssignedSO(payload){
  ...
  if (!/^\d{2}\.\d{4,5}$/.test(so))   throw new Error('SO# must be like 12.34567');
