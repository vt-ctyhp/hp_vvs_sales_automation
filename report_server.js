...
// /** File: report_server.gs (v1.2)
// *  Role: Reporting (By Status / By Rep) + dropdown lists + export.
// *  Changes (v1.2):
// *    - report_getDropdownLists_() now returns `brands` from the "Brand" column on Dropdown sheet.
// *    - report_runStatus() now accepts `payload.brands` (array) and `payload.dateFrom` / `payload.dateTo`
// *      (ISO yyyy-MM-dd strings) to filter by Brand and Visit Date Range.
// *    - New helpers: report_filterByBrands_(), report_filterByDateRange_()
// *    - report_buildSummary_() now tallies a `brand` group.
// *    - report_buildFiltersSummary_() and report_buildFriendlyExportName_() updated to reflect Brand/Date.
// *  Changes (v1.1):
// *    - Booked Appointment (Sales Stage = "Appointment"): omit "Order Total" and "Paid-to-Date" columns.
// *    - Highlight blank Assigned Rep / Assisted Rep cells in light yellow (#fff9c4) in PDF and sheet.
// */
...
//     if (i === -1) return [];
//     return dedup(rows.map(function(r){ return r[i]; }));
//   };

//   var assignedReps  = col('Assigned Rep');
//   var assistedReps0 = col('Assisted Rep');
//   var assistedReps  = assistedReps0.length ? assistedReps0 : assignedReps;
...
//   var key = 'report:master:v1:' + sh.getSheetId() + ':' + sh.getLastRow() + 'x' + sh.getLastColumn();
//   var cached = report_cacheGet_(key);
//   if (cached && cached.headers && cached.rows) return cached;
...
// function report_headerIndex_(headers) {
//   var H = {};
//   headers.forEach(function(h, i){ H[String(h)] = i; });
//   return H;
// }
...
//     var iName  = hdr.indexOf('assisted rep');
//     var iEmail = hdr.indexOf('assisted rep email');
//     if (iEmail < 0) return {};
...
//       if (!email) continue;
//       var name = String(iName >= 0 ? (values[r][iName] || '') : '').trim();
...
//     var key   = 'report:assistEmailMap:v1';
//     var hit   = cache.get(key);
//     if (hit) { try { return JSON.parse(hit); } catch (_e) {} }
//     var fresh = report_getAssistedEmailToNameMap_();
//     try {
//       var json = JSON.stringify(fresh);
//       if (json.length <= 90000) cache.put(key, json, 600);
//     } catch (_e) {}
//     return fresh;
//   } catch (_e) {
//     return report_getAssistedEmailToNameMap_();
...
//   var shaped = rows.map(function(r){ return idx.map(function(i){ return (i >= 0 ? r[i] : ''); }); });
//   return { headers: cols, rows: shaped, total: rows.length, previewLimit: 1000 };
// }
...
// // ===========================================================================================
// // NEW v1.2 — Brand filter helper
// // ===========================================================================================
// /**
//  * Filter rows to only those whose "Brand" cell matches one of the selected brands (case-insensitive).
//  * Multi-token Brand cells (comma/semicolon separated) work with OR logic.
//  * If brandSet is empty, returns rows unchanged.
//  */
// function report_filterByBrands_(headers, rows, brandArr) {
//   if (!brandArr || !brandArr.length) return rows;

//   var iBrand = report_firstHeaderIndex_(headers, ['Brand','brand']);
//   if (iBrand < 0) return rows; // column not found — pass all through

//   // Normalise selected brands to lowercase set
//   var brandSet = {};
//   brandArr.forEach(function(b){ brandSet[String(b || '').trim().toLowerCase()] = true; });

//   return rows.filter(function(r) {
//     var cell = String(r[iBrand] == null ? '' : r[iBrand]).trim();
//     if (!cell) return false;
//     // Support multi-value cells (e.g. "VVS, HP")
//     var tokens = cell.split(/[\s,;|\/&+]+/).map(function(t){ return t.toLowerCase().trim(); }).filter(Boolean);
//     for (var i = 0; i < tokens.length; i++) {
//       if (brandSet[tokens[i]]) return true;
//     }
//     // Also try the whole cell value in case brand names contain spaces
//     if (brandSet[cell.toLowerCase()]) return true;
//     return false;
//   });
// }
...
// // ===========================================================================================
// // NEW v1.2 — Date range filter helper
// // ===========================================================================================
// /**
//  * Filter rows by Visit Date within [dateFrom, dateTo] (inclusive, ISO yyyy-MM-dd strings).
//  * Blank dateFrom/dateTo means open-ended on that side.
//  * Rows with blank/unparseable Visit Dates are excluded when any date bound is active.
//  */
// function report_filterByDateRange_(headers, rows, dateFrom, dateTo) {
//   var hasFrom = (typeof dateFrom === 'string' && dateFrom.trim().length > 0);
//   var hasTo   = (typeof dateTo   === 'string' && dateTo.trim().length   > 0);
//   if (!hasFrom && !hasTo) return rows; // no filter active

//   var iVisit = headers.indexOf('Visit Date');
//   if (iVisit < 0) return rows; // column not found — pass all through

//   var tz       = Session.getScriptTimeZone();
//   var fromMs   = hasFrom ? new Date(dateFrom.trim() + 'T00:00:00').getTime() : -Infinity;
//   var toMs     = hasTo   ? new Date(dateTo.trim()   + 'T23:59:59').getTime() :  Infinity;

//   return rows.filter(function(r) {
//     var t = report_toTime_(r[iVisit]);
//     if (t == null) return false; // exclude rows with no Visit Date when a range is active
...
// // ===========================================================================================
// // Main report runner — By Status  (updated v1.2: brand + date range filtering)
// // ===========================================================================================
// function report_runStatus(payload) {
//   var data = report_getMasterData_();
//   var H    = report_headerIndex_(data.headers);

//   // 1) Collapse to latest row per rootapptid
//   var latestRows = report_pickLatestRowsByRootApptId_(data.headers, data.rows);

//   // 2) Status filters
//   var sets = {
//     salesStage:        new Set(payload && payload.salesStage        ? payload.salesStage        : []),
//     conversionStatus:  new Set(payload && payload.conversionStatus  ? payload.conversionStatus  : []),
//     customOrderStatus: new Set(payload && payload.customOrderStatus ? payload.customOrderStatus : []),
//     centerStoneStatus: new Set(payload && payload.centerStoneStatus ? payload.centerStoneStatus : [])
//   };

//   var out = [];
//   for (var ri = 0; ri < latestRows.length; ri++) {
//     var r      = latestRows[ri];
//     var vStage  = String(r[H['Sales Stage']]              || '').trim();
//     var vConv   = String(r[H['Conversion Status']]        || '').trim();
//     var vCust   = String(r[H['Custom Order Status']]      || '').trim();
//     var vCenter = String(r[H['Center Stone Order Status']]|| '').trim();

//     var mStage  = sets.salesStage.size        ? sets.salesStage.has(vStage)         : true;
//     var mConv   = sets.conversionStatus.size  ? sets.conversionStatus.has(vConv)    : true;
//     var mCust   = sets.customOrderStatus.size ? sets.customOrderStatus.has(vCust)   : true;
//     var mCenter = sets.centerStoneStatus.size ? sets.centerStoneStatus.has(vCenter) : true;

//     if (mStage && mConv && mCust && mCenter) out.push(r);
//   }

//   // 3) NEW: Brand filter
//   out = report_filterByBrands_(data.headers, out, payload && payload.brands ? payload.brands : []);

//   // 4) NEW: Date range filter (Visit Date)
//   out = report_filterByDateRange_(
//     data.headers, out,
//     payload && payload.dateFrom ? payload.dateFrom : '',
//     payload && payload.dateTo   ? payload.dateTo   : ''
//   );

//   // 5) Sort by Visit Date ascending (blanks last)
//   var vi = data.headers.indexOf('Visit Date');
//   if (vi !== -1) {
//     out.sort(function(a, b) {
//       var ta = report_toTime_(a[vi]);
...
//   // 7) Preview formatting
//   if (payload && payload._mode === 'preview') {
//     var dtIdx = shaped.headers.indexOf('Visit Date');
//     var otIdx = shaped.headers.indexOf('Order Total');
//     var ptIdx = shaped.headers.indexOf('Total Pay To Date');
//     var pdIdx = shaped.headers.indexOf('Production Deadline');

//     for (var i = 0; i < shaped.rows.length; i++) {
//       if (dtIdx !== -1) shaped.rows[i][dtIdx] = report_formatDateYMD_(shaped.rows[i][dtIdx]);
//       if (otIdx !== -1) shaped.rows[i][otIdx] = report_fmtCurrency0_(shaped.rows[i][otIdx]);
//       if (ptIdx !== -1) shaped.rows[i][ptIdx] = report_fmtCurrency0_(shaped.rows[i][ptIdx]);
//       if (pdIdx !== -1) shaped.rows[i][pdIdx] = report_formatDateYMD_(shaped.rows[i][pdIdx]);
//     }
//     shaped.rows = shaped.rows.slice(0, Math.min(shaped.previewLimit || 1000, shaped.rows.length));
//   }
...
// // ===========================================================================================
// // Main report runner — By Rep  (unchanged except summary now includes brand)
// // ===========================================================================================
// function report_runReps(payload) {
//   var data = report_getMasterData_();
//   var H    = report_headerIndex_(data.headers);
...
//   var assignedSet    = new Set((payload && payload.assigned || []).map(String));
//   var assistedSet    = new Set((payload && payload.assisted || []).map(String));
//   var anyRepSelected = (assignedSet.size + assistedSet.size) > 0;

//   var stageSet  = new Set((payload && payload.salesStage        || []).map(String));
//   var convSet   = new Set((payload && payload.conversionStatus  || []).map(String));
//   var customSet = new Set((payload && payload.customOrderStatus || []).map(String));
//   var centerSet = new Set((payload && payload.centerStoneStatus || []).map(String));

//   var out = [];
//   for (var ri = 0; ri < latestRows.length; ri++) {
//     var r = latestRows[ri];

//     var assignedTokens = report_tokenizeMulti_(r[H['Assigned Rep']]);
//     var assistedTokens = report_tokenizeMulti_(r[H['Assisted Rep']]);
//     var assignedMatch  = assignedSet.size ? assignedTokens.some(function(t){ return assignedSet.has(t); }) : false;
//     var assistedMatch  = assistedSet.size ? assistedTokens.some(function(t){ return assistedSet.has(t); }) : false;
//     var repMatch       = anyRepSelected ? (assignedMatch || assistedMatch) : true;
...
//     var stageMatch  = stageSet.size  ? stageSet.has(vStage)  : true;
//     var convMatch   = convSet.size   ? convSet.has(vConv)    : true;
//     var customMatch = customSet.size ? customSet.has(vCust)  : true;
//     var centerMatch = centerSet.size ? centerSet.has(vCenter): true;
...
//   if (type === 'status') {
//     var parts = [];
//     var keys  = ['salesStage','conversionStatus','customOrderStatus','centerStoneStatus'];
//     for (var i = 0; i < keys.length; i++) {
//       var k = keys[i];
//       if (Array.isArray(payload && payload[k]) && payload[k].length) parts.push(payload[k].slice(0,2).join('|'));
//     }
//     if (payload && Array.isArray(payload.brands) && payload.brands.length) parts.push('Brand:' + payload.brands.slice(0,2).join('|'));
//     if (payload && payload.dateFrom) parts.push(payload.dateFrom + (payload.dateTo ? '→'+payload.dateTo : ''));
//     return parts.join(' • ') || 'All';
//   }
//   if (type === 'rep') {
//     var A = (payload && payload.assigned || []).slice(0,2).join('|');
//     var S = (payload && payload.assisted || []).slice(0,2).join('|');
//     var ps = [];
//     if (A) ps.push('A:'+A);
//     if (S) ps.push('S:'+S);
//     return ps.join(' • ') || 'All';
...
// function report_buildSheetName_(type, payload) {
//   var title = (type === 'status') ? 'By Status' : 'By Rep';
//   var now   = Utilities.formatDate(new Date(), Session.getScriptTimeZone(), 'yyyy-MM-dd HH.mm');
//   var crit  = report_shortCriteria_(type, payload) || 'All';
//   var name  = 'Report — ' + title + ' — ' + crit + ' — ' + now;
//   if (name.length <= 100) return name;
//   var hash    = Utilities.base64EncodeWebSafe(
//     Utilities.computeDigest(Utilities.DigestAlgorithm.MD5, JSON.stringify(payload))
//   ).slice(0,4);
//   var compact = 'Report — ' + title + ' — ' + now + ' — ' + hash;
...
//   var DISPLAY_MAP = { 'Appointment': 'Booked Appointment' };
//   if (DISPLAY_MAP[chosenStatus]) chosenStatus = DISPLAY_MAP[chosenStatus];

//   // NEW v1.2: append brand suffix if brand(s) selected
//   var brandSuffix = '';
//   if (payload && Array.isArray(payload.brands) && payload.brands.length) {
//     brandSuffix = ' [' + payload.brands.join('+') + ']';
//   }

//   // NEW v1.2: append date range suffix
//   var dateSuffix = '';
//   if (payload && (payload.dateFrom || payload.dateTo)) {
//     dateSuffix = ' (' + (payload.dateFrom || '…') + ' to ' + (payload.dateTo || '…') + ')';
//   }

//   var special  = { 'deposit paid': true, 'in production': true, 'viewing scheduled': true };
//   var isSpecial = !!special[chosenStatus.toLowerCase()];
...
//       if (iAssigned !== -1 && Array.isArray(res.rows) && res.rows.length) {
//         var bag = {};
//         res.rows.forEach(function(r){ var v = String(r[iAssigned]||'').trim(); if (v) bag[v] = true; });
//         var keys = Object.keys(bag);
//         if (keys.length === 1) selectedRep = keys[0];
//       }
//     }
//     if (selectedRep) repPrefix = selectedRep + ': ';
//   }
...
//   var title = (repPrefix ? repPrefix : '') + chosenStatus + brandSuffix + dateSuffix + ' - ' + ts + (user ? (' (gen. by ' + user + ')') : '');
//   if (title.length > 100) title = title.slice(0, 100);
//   return title;
// }
...
//   if (/client status report url/i.test(h)) {
//     var url = String(s || '').trim();
//     if (/^https?:\/\//i.test(url)) return '<a href="' + report_htmlEscape_(url) + '" target="_blank">Open</a>';
//     return '';
//   }
//   var str = String(s);
//   if (/^https?:\/\//i.test(str)) return '<a href="' + report_htmlEscape_(str) + '" target="_blank">' + report_htmlEscape_(str) + '</a>';
//   return report_htmlEscape_(str);
// }

// // Updated v1.2: includes Brand and Date Range in the filter summary line
// function report_buildFiltersSummary_(type, payload) {
//   try {
//     if (type === 'status') {
//       var parts = [];
...
//       add('Custom Order',  payload && payload.customOrderStatus);
//       add('Center Stone',  payload && payload.centerStoneStatus);
//       add('Brand',         payload && payload.brands);
//       // Date range
//       var df = (payload && payload.dateFrom) ? payload.dateFrom : '';
//       var dt = (payload && payload.dateTo)   ? payload.dateTo   : '';
//       if (df || dt) parts.push('Visit Date: ' + (df || '…') + ' → ' + (dt || '…'));
//       return parts.join(' • ');
//     }
//     if (type === 'rep') {
//       var A = (payload && Array.isArray(payload.assigned) && payload.assigned.length) ? payload.assigned.join(', ') : 'All';
...
//   function normStatus(x){ return String(x == null ? '' : x).replace(/\u00A0/g,' ').trim().toLowerCase(); }
...
//       var h = res.headers[c];
//       var v = r[c];
//       var bgColor = null, fontClr = null;

//       if (c === idxPD && idxPD >= 0 && status === 'in production') {
//         var ms      = report_toTime_(v);
//         var ymd     = report_formatDateYMD_(v);
//         var overdue = (ms != null) ? (ms < todayMs) : (ymd && ymd < todayYMD);
//         if (overdue) { bgColor = '#ff3b30'; fontClr = '#000000'; }
//       }

//       if (highlightBlankReps && (c === idxAssigned || c === idxAssist)) {
//         if (String(v == null ? '' : v).trim() === '') bgColor = '#fff9c4';
//       }

//       if (c === idxAck && idxAck >= 0) {
//         if (/needs\s*follow[-\s]?up/i.test(String(v == null ? '' : v).trim())) { bgColor = '#ffa726'; fontClr = '#000000'; }
//       }

//       if (c === idxDays && idxDays >= 0) {
//         var dnum = Number(v);
//         if (isFinite(dnum) && dnum >= 3) { bgColor = '#ffa726'; fontClr = '#000000'; }
//       }

//       var tdStyle = '', tdBgAttr = '';
//       if (bgColor) {
//         tdStyle  = ' style="background-color:' + bgColor + (fontClr ? ';color:' + fontClr : '') + ';"';
//         tdBgAttr = ' bgcolor="' + bgColor + '"';
//       }
...
//     Logger.log('[exportPdf] blobBytes=%s mime=%s',
//                (blob && blob.getBytes && blob.getBytes().length) || -1, blob && blob.getContentType());
...
//   var otIdx = res.headers.indexOf('Order Total');
//   var ptIdx = res.headers.indexOf('Paid-to-Date');
//   if (ptIdx === -1) ptIdx = res.headers.indexOf('Total Pay To Date');

//   if (lastRow > 1) {
//     if (otIdx !== -1) sh.getRange(2, otIdx+1, lastRow-1, 1).setNumberFormat('$#,##0');
//     if (ptIdx !== -1) sh.getRange(2, ptIdx+1, lastRow-1, 1).setNumberFormat('$#,##0');
//   }

//   var dIdx = res.headers.indexOf('Days Since Last Update');
//   if (dIdx !== -1 && lastRow > 1) sh.getRange(2, dIdx+1, lastRow-1, 1).setNumberFormat('0');

//   var cosIdx = res.headers.indexOf('Custom Order Status');
//   var pdIdx  = res.headers.indexOf('Production Deadline');
//   if (lastRow > 1 && cosIdx !== -1 && pdIdx !== -1) {
//     var nRows  = lastRow - 1;
//     var cosVals = sh.getRange(2, cosIdx+1, nRows, 1).getValues();
//     var pdVals  = sh.getRange(2, pdIdx +1, nRows, 1).getValues();
//     var bg = new Array(nRows), fc = new Array(nRows), fw = new Array(nRows);
//     var tz      = Session.getScriptTimeZone();
//     var todayYMD = Utilities.formatDate(new Date(), tz, 'yyyy-MM-dd');
//     var todayMs  = new Date(todayYMD + 'T00:00:00').getTime();
...
//       var pv      = pdVals[i][0];
//       var ms      = report_toTime_(pv);
//       var ymd     = (function(v){ var s=(v==null?'':String(v)).trim(); if(!s) return ''; var d=new Date(s); return isNaN(d)?s:Utilities.formatDate(d,tz,'yyyy-MM-dd'); })(pv);
//       var overdue = (ms != null) ? (ms < todayMs) : (ymd && ymd < todayYMD);
//       var hit     = (status === 'in production') && overdue;
//       bg[i] = [hit ? '#ff3b30' : null];
//       fc[i] = [hit ? '#000000' : null];
//       fw[i] = ['normal'];
//     }
//     sh.getRange(2, pdIdx+1, nRows, 1).setBackgrounds(bg).setFontColors(fc).setFontWeights(fw).setNumberFormat('yyyy-mm-dd');
//   }

//   var pdIdx2 = res.headers.indexOf('Production Deadline');
//   if (pdIdx2 !== -1 && lastRow > 1) sh.getRange(2, pdIdx2+1, lastRow-1, 1).setNumberFormat('yyyy-mm-dd');
...
//     var DATA_ROW_HEIGHT = 48;
//     var start = 2, remain = lastRow-1, BATCH = 500;
//     while (remain > 0) { var n = Math.min(BATCH, remain); sh.setRowHeights(start, n, DATA_ROW_HEIGHT); start+=n; remain-=n; }
//     SpreadsheetApp.flush();
//   }
...
//     var idxAssigned = res.headers.indexOf('Assigned Rep');
//     var idxAssist   = res.headers.indexOf('Assisted Rep');
//     var nRows2 = lastRow - 1;
//     if (idxAssigned !== -1) {
//       var valsA = sh.getRange(2, idxAssigned+1, nRows2, 1).getDisplayValues();
//       var bgsA  = valsA.map(function(row){ return [String(row[0]||'').trim() ? null : '#fff9c4']; });
//       sh.getRange(2, idxAssigned+1, nRows2, 1).setBackgrounds(bgsA);
//     }
//     if (idxAssist !== -1) {
//       var valsS = sh.getRange(2, idxAssist+1, nRows2, 1).getDisplayValues();
//       var bgsS  = valsS.map(function(row){ return [String(row[0]||'').trim() ? null : '#fff9c4']; });
//       sh.getRange(2, idxAssist+1, nRows2, 1).setBackgrounds(bgsS);
//     }
//   }

//   var idxAck2  = res.headers.indexOf('Ack Status');
//   var idxDays2 = res.headers.indexOf('Days Since Last Update');
//   if (lastRow > 1) {
//     var n2 = lastRow - 1;
//     if (idxAck2 !== -1) {
//       var ackVals = sh.getRange(2, idxAck2+1, n2, 1).getDisplayValues();
//       var bgsAck = new Array(n2), fcsAck = new Array(n2);
//       for (var i2 = 0; i2 < n2; i2++) {
//         var a = String(ackVals[i2][0]||'').trim();
//         var hitAck = /needs\s*follow[-\s]?up/i.test(a);
//         bgsAck[i2] = [hitAck ? '#ffa726' : null];
//         fcsAck[i2] = [hitAck ? '#000000' : null];
//       }
//       sh.getRange(2, idxAck2+1, n2, 1).setBackgrounds(bgsAck).setFontColors(fcsAck);
//     }
//     if (idxDays2 !== -1) {
//       var daysVals = sh.getRange(2, idxDays2+1, n2, 1).getValues();
//       var bgsDays = new Array(n2), fcsDays = new Array(n2);
//       for (var j2 = 0; j2 < n2; j2++) {
//         var d = Number(daysVals[j2][0] || 0);
//         var hitDays = isFinite(d) && d >= 3;
//         bgsDays[j2] = [hitDays ? '#ffa726' : null];
//         fcsDays[j2] = [hitDays ? '#000000' : null];
//       }
//       sh.getRange(2, idxDays2+1, n2, 1).setBackgrounds(bgsDays).setFontColors(fcsDays);
//     }
//   }
...
// // Updated v1.2: tallies brand
// function report_buildSummary_(headers, rows) {
//   var H = {};
//   headers.forEach(function(h, i){ H[h] = i; });
//   var salesStages = [], conversions = [], customs = [], centers = [], unifiedReps = [], brands = [];
...
//     customs.push(r[H['Custom Order Status']]);
//     centers.push(r[H['Center Stone Order Status']]);
//     brands.push(r[H['Brand']]);

//     var assigned = report_tokenizeMulti_(r[H['Assigned Rep']]);
//     var assisted = report_tokenizeMulti_(r[H['Assisted Rep']]);
//     var bag = {};
//     assigned.concat(assisted).forEach(function(t){ bag[t] = true; });
//     var bagKeys = Object.keys(bag);
//     if (bagKeys.length === 0) bagKeys = ['(blank)'];
//     bagKeys.forEach(function(rep){ unifiedReps.push(rep); });
//   }
...
//       customOrderStatus:report_countMap_(customs),
//       centerStoneStatus:report_countMap_(centers),
//       brand:            report_countMap_(brands),      // ← NEW v1.2
//     }
//   };
// }

// function report_buildSummarySheetNameFromBase_(baseName) {
//   var name1 = baseName + ' — Summary';
//   if (name1.length <= 100) return name1;
//   var name2 = 'Summary — ' + baseName;
//   if (name2.length <= 100) return name2;
//   return name2.slice(0,100);
// }
...
// function report_writeSummarySheet_(ss, baseName, summary) {
//   if (!summary || !summary.groups) return null;
//   var name = report_buildSummarySheetNameFromBase_(baseName);
...
//     ['Custom Order Status',        'customOrderStatus'],
//     ['Center Stone Order Status',  'centerStoneStatus'],
//     ['Brand',                      'brand'],            // ← NEW v1.2
//   ];

//   var row = 1;
//   for (var i = 0; i < sections.length; i++) {
//     var label = sections[i][0], key = sections[i][1];
//     var map   = summary.groups[key] || {};
...
//     sh.getRange(row, 1).setValue(label).setFontWeight('bold'); row++;
//     sh.getRange(row, 1, 1, 2).setValues([['Value','Count']]).setFontWeight('bold').setBackground('#f5f5f5'); row++;
//     if (entries.length) { sh.getRange(row, 1, entries.length, 2).setValues(entries); row += entries.length; }
//     row++;
//   }
//   var lastRow = sh.getLastRow();
...
// // ===========================================================================================
// function report_cache_()           { return CacheService.getUserCache(); }
// function report_cacheGet_(key)     { try { var s = report_cache_().get(key); return s ? JSON.parse(s) : null; } catch(e) { return null; } }
// function report_cachePut_(key, obj, ttlSec) {
//   try {
...
// function report_sortRowsByVisitDate_(res) {
//   var col = res.headers.indexOf('Visit Date');
//   if (col === -1) return res;
//   function ts(v){
//     if (v==null||v==='') return Infinity;
//     if (Object.prototype.toString.call(v)==='[object Date]'){ var t=v.getTime(); return isNaN(t)?Infinity:t; }
//     var d=new Date(v); return isNaN(d.getTime())?Infinity:d.getTime();
//   }
//   res.rows.sort(function(a,b){ return ts(a[col])-ts(b[col]); });
//   return res;
// }
...
// function report_setHyperlinkLabel_(sh, res, headerName, label) {
//   var colIdx = res.headers.indexOf(headerName);
//   if (colIdx === -1) return;
//   var lastRow = sh.getLastRow();
//   if (lastRow < 2) return;
//   var rng  = sh.getRange(2, colIdx+1, lastRow-1, 1);
...
// function report_getGeneratorEmail_() {
//   if (typeof REPORT_EXPORT_USER === 'string' && REPORT_EXPORT_USER.trim()) return REPORT_EXPORT_USER.trim();
//   var u = '';
//   try { u = Session.getActiveUser().getEmail() || ''; } catch (e) {}
...
//   Logger.log('[quickStatus] payload=%s', JSON.stringify(payload));
//   var out = report_exportPdf('status', payload);
//   Logger.log('[quickStatus] export ok=%s file="%s" err=%s', !!(out && out.ok), out && out.fileName, out && out.error);
//   if (!out || !out.ok) throw new Error(out && out.error ? out.error : 'Export failed.');
//   report_showDownloadDialog_(out.fileName, out.bytesBase64, out.mimeType || 'application/pdf');
// }
...
// '  if(document.readyState==="loading"){ document.addEventListener("DOMContentLoaded", start); } else { setTimeout(start,0); }' +
// '})();</script></body></html>';
//   SpreadsheetApp.getUi().showModalDialog(HtmlService.createHtmlOutput(html).setWidth(380).setHeight(180), 'PDF Export');
// }
...
// ===========================================================================================
// v1.3 - Date range filter helper (Closed/Won Date = Updated At)
// ===========================================================================================
/**
 * Filter rows by Updated At - the date rep last updated the record (e.g. marked Won/Closed).
 * Range is [dateFrom, dateTo] inclusive (ISO yyyy-MM-dd strings).
 * Blank dateFrom/dateTo means open-ended on that side.
 * Rows with blank/unparseable Updated At are excluded when any date bound is active.
 */
function report_filterByDateRange_(headers, rows, dateFrom, dateTo) {
  var hasFrom = (typeof dateFrom === "string" && dateFrom.trim().length > 0);
  var hasTo   = (typeof dateTo   === "string" && dateTo.trim().length   > 0);
  ...
  // Updated At = timestamp when rep last updated the record (closed/won date)
  var iUpdated = report_firstHeaderIndex_(headers, [
    "Updated At", "Last Updated At", "Last Updated", "UpdatedAt", "Updated At (ISO)"
  ]);
  if (iUpdated < 0) return rows; // column not found - pass all through

  var fromMs = hasFrom ? new Date(dateFrom.trim() + "T00:00:00").getTime() : -Infinity;
  var toMs   = hasTo   ? new Date(dateTo.trim()   + "T23:59:59").getTime() :  Infinity;

  return rows.filter(function(r) {
    var t = report_toTime_(r[iUpdated]);
    if (t == null) return false; // exclude rows with no Updated At when range is active
    return (t >= fromMs && t <= toMs);
  });
}
...
function report_buildFiltersSummary_(type, payload) {
  try {
    if (type === 'status') {
      ...
      var df = (payload && payload.dateFrom) ? payload.dateFrom : '';
      var dt = (payload && payload.dateTo)   ? payload.dateTo   : '';
      if (df || dt) parts.push('Closed/Won Date: ' + (df || '...') + ' to ' + (dt || '...'));
      return parts.join(' • ');
    }
    if (type === 'rep') {
      var A = (payload && Array.isArray(payload.assigned) && payload.assigned.length) ? payload.assigned.join(', ') : 'All';
